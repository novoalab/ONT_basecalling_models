...
dir_human <- setwd(here("..","__data","rRNA","modkit","inosine_m6A","A_only","human"))

dir_mouse <- setwd(here("..","__data","rRNA","modkit","inosine_m6A","A_only","mouse"))

dir_ecoli <- setwd(here("..","__data","rRNA","modkit","inosine_m6A","A_only","ecoli"))
...
read_dir <- function(file_list, work_dir){
  ...
  for (i in 1:length(file_list)){
    temp_data <- read_tsv(file_list[i], col_names = T) #each file will be read in, specify which columns you need read in to avoid any errors # specifying col_types is essential to see spike_ins
...
filter_table <- function(df, ref){
  ...
  df_filt <- df %>% 
    #keep only m6A predictions (not inosine)
    filter(name == "a") %>% 
    #change a to m6A to match ref annotation
    mutate(name = if_else(name == "a", "m6A", name)) %>%
    separate(sample, into = c("condition","replicate")) %>% 
    select("chr" = chrom, "start" = chromStart, "end" = chromEnd, "Nval" = score, "percent_mod" = percent_modified, condition, replicate) %>% 
...
write.table(master_tbl, file= here("..","__results_Gregor","rRNA","tables","master_table_filtered_m6A.tsv"), sep = "\t", row.names = F)
...
ecoli_filt %>% 
  ...
  scale_color_manual(values = c("m6A" = "#381a61", "A_mod_non_m6A" = "#32CD32", "mod_adjacent" = "#C71585", "unmodified" = "grey60")) +
  xlim(0, 100) +
  ylim(0, 100) +
  theme_minimal() +
...
ggsave(filename = here("..","__results_Gregor","rRNA","wt_vs_ivt", "m6A","scatterplots","human_23S_16S_rep1.pdf"),       
       plot = last_plot(), 
       device = "pdf", 
...
human_filt %>%
  ...
  scale_fill_manual(values = c("m6A" = "#381a61", "A_mod_non_m6A" = "#32CD32", "mod_adjacent" = "#C71585", "unmodified" = "grey60")) +
  stat_n_text() +
  coord_flip() +
  theme_pubr() + t

ggsave(filename = here("..","__results_Gregor","rRNA","wt_vs_ivt", "m6A","boxlpots_wt_amounts","human_28_18_rep1_all_mods_simplified.pdf"),       
       plot = last_plot(), 
       device = "pdf", 
       width = 6, height = 6, units = "in")
...
master_df$mod_simplified <- factor(master_df$mod_simplified, levels = c("unmodified", "mod_adjacent","A_mod_non_m6A","m6A"))
...
master_df %>% 
  ...
  labs(
    ...
    y = "Proportion",
    fill = "Modkit",
    title = "Filtered Modkit [wt > 10%]"
  ) + theme_pubr() + t


ggsave(filename = here("..","__results_Gregor","rRNA","wt_vs_ivt", "m6A","stacked_barchart","all_species_wt+ivt_filter_rep1_simplified.pdf"),       
       plot = last_plot(), 
       device = "pdf", 
