...
write.table(master_tbl, file= here("..","master_table_filtered_m5C.tsv"))
...
ecoli_filt %>% 
  ...
  scale_color_manual(values = c("m5C" = "#f9d14a", "C_mod_non_m5C" = "#32CD32", "mod_adjacent" = "#C71585", "unmodified" = "grey60")) +
  xlim(0, 100) +
  ylim(0, 100) +
  theme_minimal() +
...
ggsave(filename = here("..","__results_Gregor","rRNA","wt_vs_ivt", "m5C","scatterplots","human_28S_18S_rep1.pdf"),       
       plot = last_plot(), 
       device = "pdf", 
...
ggsave(filename = here("..","__results_Gregor","rRNA","wt_vs_ivt", "m5C","boxlpots_wt_amounts","human_28S_18S_rep1_simplified_all_mods.pdf"),       
       plot = last_plot(), 
       device = "pdf", 
       width = 6, height = 6, units = "in")
...
human_summary <- human_filt %>% 
  ...
  # filter out 0s introduced by including ivt sites! for summary stats
  filter(wt > 0 & ivt > 0) %>% 
...
master_df$mod_simplified <- factor(master_df$mod_simplified, levels = c("unmodified", "mod_adjacent","C_mod_non_m5C","m5C"))
...
master_df %>% 
  ...
  labs(
    x = NULL,
    y = "Proportion",
    fill = "Modkit [> 10% wt-cutoff | <5% ivt-cutoff]",
    title = "Unfiltered Modkit"
  ) + theme_pubr() + t


ggsave(filename = here("..","__results_Gregor","rRNA","wt_vs_ivt", "m5C","stacked_barchart","all_species_ivt+wt_filter_28S-23S_18S-16S_rep1_simplified.pdf"),       
       plot = last_plot(), 
       device = "pdf", 
